/*
  ******************************************************************************
  * @file      startup_stm32h755xx_CM4.s
  * @author    MCD Application Team (fixed for CM4 by Giulio)
  * @brief     STM32H755xx CM4 core vector table for GCC toolchain.
  *
  * Fixes vs original:
  *   1. .cpu cortex-m4       (was cortex-m7)
  *   2. .fpu fpv4-sp-d16     (was softvfp — enables hardware FPU)
  *   3. VTOR set to 0x08100000 BEFORE SystemInit (which resets it to 0x08000000)
  *   4. VTOR re-asserted AFTER SystemInit returns
  *   5. FPU CPACR CP10+CP11 enabled before any C code runs
  *   6. SHCSR: MemManage, BusFault, UsageFault handlers explicitly enabled
  *   7. Default_Handler: bkpt #0 for debugger halt instead of silent loop
  ******************************************************************************
  * @attention
  *
  ...
  */
  ...
  .syntax unified
  .cpu cortex-m4          /* FIX 1: was cortex-m7                          */
  .fpu fpv4-sp-d16        /* FIX 2: was softvfp — CM4 has hardware FPv4-SP */
  .thumb
...
/**
 * @brief  Reset_Handler — CM4 entry point
 */
...
Reset_Handler:
  ...
  /* ------------------------------------------------------------------ */
  /* FIX 3: Set VTOR to CM4 flash bank 0x08100000 as the VERY FIRST     */
  /* operation, before any function call.                                */
  /*                                                                     */
  /* SystemInit() in system_stm32h7xx.c unconditionally writes:         */
  /*   SCB->VTOR = FLASH_BANK1_BASE (0x08000000)                        */
  /* which points the CM4 at the CM7 vector table. All handler          */
  /* addresses read from that table are CM7 code bytes — garbage for    */
  /* CM4 — causing LOCKUP the moment any fault or interrupt fires.      */
  /* ------------------------------------------------------------------ */
  ldr   r0, =0x08100000
  ldr   r1, =0xE000ED08    /* SCB->VTOR */
  str   r0, [r1]
  dsb                       /* data sync: ensure write visible before fetch */
  isb                       /* instruction sync: flush pipeline              */

  /* ------------------------------------------------------------------ */
  /* FIX 4: Enable CM4 hardware FPU — CP10 + CP11 full access           */
  /*                                                                     */
  /* Without this, any FPU instruction triggers NOCP UsageFault, which  */
  /* escalates to HardFault (FORCED) since UsageFault handler is weak.  */
  /* The compiler emits FPU instructions for all float operations when  */
  /* built with -mfpu=fpv4-sp-d16 -mfloat-abi=hard.                    */
  /* ------------------------------------------------------------------ */
  ldr   r0, =0xE000ED88    /* CPACR */
  ldr   r1, [r0]
  orr   r1, r1, #(0xF << 20)  /* CP10[21:20]=11, CP11[23:22]=11 = full access */
  str   r1, [r0]
  dsb
  isb

  /* ------------------------------------------------------------------ */
  /* FIX 5: Unmask MemManage, BusFault, UsageFault handlers             */
  /*                                                                     */
  /* By default SHCSR[18:16] = 0, so all three fault types escalate     */
  /* silently to HardFault (FORCED), making root cause impossible to    */
  /* distinguish from the HardFault analyzer. Enabling them here means  */
  /* each fault type fires its own handler and shows the correct flags. */
  /* ------------------------------------------------------------------ */
  ldr   r0, =0xE000ED24    /* SCB->SHCSR */
  ldr   r1, [r0]
  orr   r1, r1, #0x00070000  /* bit18=USGFAULTENA, 17=BUSFAULTENA, 16=MEMFAULTENA */
  str   r1, [r0]

  /* Set initial stack pointer */
  ldr   sp, =_estack

  /* Configure power supply (required on STM32H7 before clock init) */
  bl    ExitRun0Mode

  /* SystemInit configures clocks but ALSO resets VTOR to 0x08000000   */
  bl    SystemInit

  /* FIX 3b: Re-assert VTOR immediately after SystemInit overwrites it  */
  ldr   r0, =0x08100000
  ldr   r1, =0xE000ED08
  str   r0, [r1]
  dsb
 ...
 *
 * FIX 7: bkpt #0 halts execution here when a debugger is attached,
 * showing you exactly which unhandled interrupt/exception fired.
 * Without a debugger the CPU resumes the loop as before.
 */
...
/*******************************************************************************
* Vector table
* Placed in .isr_vector → lands at 0x08100000 (start of CM4 flash bank)
* as guaranteed by the linker script KEEP(*(.isr_vector)) >FLASH
*******************************************************************************/
...
/*******************************************************************************
* Weak aliases — any C function with the same name overrides these
*******************************************************************************/
  .weak      NMI_Handler
