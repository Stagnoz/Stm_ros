/* Test T6j: Call lwip_init() instead of tcpip_init() */
#include "main.h"
#include "cmsis_os.h"
#include "lwip/init.h"  /* Use lwip_init instead of tcpip_init */
...
void StartDefaultTask(void *argument)
{
  ...
  /* Call lwip_init instead of tcpip_init */
  lwip_init();
  
  /* 4 blinks GREEN = after lwip_init */
  for(int j = 0; j < 4; j++) {
    GPIOB->BSRR = (1 << 0);
    for(volatile int i = 0; i < 2000000; i++);
    GPIOB->BSRR = (1 << 16);
