/* Test T6h: Use busy-wait in main loop (no osDelay) */
#include "main.h"
#include "cmsis_os.h"
...
void StartDefaultTask(void *argument)
{
  ...
  /* 4 blinks GREEN = after tcpip_init */
  for(int j = 0; j < 4; j++) {
    GPIOB->BSRR = (1 << 0);
    for(volatile int i = 0; i < 2000000; i++);
    GPIOB->BSRR = (1 << 16);
  ...
  /* Main loop - BUSY WAIT instead of osDelay */
  for(;;)
  {
    GPIOB->BSRR = (1 << 0);    // GREEN ON
    for(volatile int i = 0; i < 2000000; i++);
    GPIOB->BSRR = (1 << 16);   // GREEN OFF
    for(volatile int i = 0; i < 2000000; i++);
  }
}
